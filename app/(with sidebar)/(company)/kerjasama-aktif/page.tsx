    ...
    async function fetchData() {
      try {
        const [offersRes, pitchesRes] = await Promise.all([
          apiCall<{ data: any[] }>("/offers/my", {}).catch(() => ({ data: [] })),
          apiCall<{ data: any[] }>("/pitches/incoming", {}).catch(() => ({ data: [] }))
        ]);

        const offers = Array.isArray(offersRes?.data) ? offersRes.data : [];
        const pitches = Array.isArray(pitchesRes?.data) ? pitchesRes.data : [];

        // Combine and filter for only accepted/approved deals
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1">Kelola dan pantau seluruh kerjasama event Anda yang sedang berlangsung.</p>
      </div>
