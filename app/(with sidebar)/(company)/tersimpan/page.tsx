...
import EventCardTersimpan from "@/components/tersimpan/event-card-tersimpan";
    ...
    const fetchEvents = async () => {
      try {
        ...
        const response = await apiCall<{
          data: ApiEvent[];
          meta: { total: number };
        }>("/saved-events", { requireAuth: true });

        console.log("response: ", response);
        const transformedEvents: EventCardTersimpanProps[] = response.data.map(
          (event: ApiEvent) => {
            ...
            return {
              ...
              organizerAvatar: event.eoProfile.logoUrl || "/avatar-1.png",
              date: dateRange,
              location: event.city,
            };
          },
