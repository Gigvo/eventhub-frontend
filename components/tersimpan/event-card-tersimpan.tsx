...
export default function EventCardTersimpan({
  ...
  const handleSave = async () => {
    try {
      if (save) {
        await apiCall(`/saved-events/${id}`, { method: "DELETE" });
        setSave(false);
        toast.success("Event dihapus dari tersimpan!");
        if (onUnsave) onUnsave(id);
      } else {
        await apiCall(`/saved-events/${id}`, { method: "POST" });
        setSave(true);
        toast.success("Event berhasil disimpan!");
      }
    } catch (error) {
      console.error("Failed to update saved status:", error);
      toast.error("Gagal memperbarui status tersimpan");
    }
  };
